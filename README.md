...
* $\mathbf{r} = [r_a, r_b, r_c, r_d]^T$ be the vector of attributed revenue per unit budget for each campaign where $\mathbf{r} = (1 + \text{return on investment (per-unit)}) \times \text{cost}$
...
* Total budget constraint (assuming total budget = $1 million): $$\mathbf{1}^T \mathbf{x} = \text{total budget}$$
